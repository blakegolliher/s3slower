...
// openNewFile opens a new log file with a timestamp-based name. If a same-
// second restart collides with an existing file that would already exceed
// maxSize, a numeric suffix is appended so writes never resume in an
...
func (l *RotatingLogger) openNewFile() error {
	...
	for suffix := 0; ; suffix++ {
		...
		if info.Size() >= l.maxSize && suffix < 1000 {
			f.Close()
			continue
		}
