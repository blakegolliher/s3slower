...
// Debug event for understanding what's happening
...
BPF_PERF_OUTPUT(debug_events);  // Debug output channel
...
int trace_write(struct pt_regs *ctx) {
    ...
    // Always emit debug event for known clients
    if (client_type != CLIENT_UNKNOWN) {
        struct debug_event_t debug = {};
        debug.ts_us = bpf_ktime_get_ns() / 1000;
        debug.pid = pid;
        debug.tid = tid;
        debug.fd = fd;
        debug.event_type = DEBUG_WRITE_SEEN;
    ...
    if (is_http_request(data)) {
        // Emit debug event for HTTP request
        if (client_type != CLIENT_UNKNOWN) {
            struct debug_event_t debug = {};
            debug.ts_us = bpf_ktime_get_ns() / 1000;
            debug.pid = pid;
            debug.tid = tid;
            debug.fd = fd;
            debug.event_type = DEBUG_HTTP_REQUEST;
            debug.client_type = client_type;
            debug.size = count;
...
int trace_read(struct pt_regs *ctx) {
    ...
    if (client_type != CLIENT_UNKNOWN) {
        ...
        debug.tid = tid;
        debug.fd = fd;
        debug.event_type = DEBUG_READ_SEEN;
        debug.client_type = client_type;
        debug.size = count;
    ...
    // Emit debug event for HTTP response
    if (client_type != CLIENT_UNKNOWN) {
        struct debug_event_t debug = {};
        debug.ts_us = bpf_ktime_get_ns() / 1000;
        debug.pid = pid;
        debug.tid = tid;
        debug.fd = fd;
        debug.event_type = DEBUG_HTTP_RESPONSE;
        debug.client_type = client_type;
        debug.size = count;
